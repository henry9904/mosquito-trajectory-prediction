# -*- coding: utf-8 -*-
"""
train_v3_aug.py — v3_D + Noise Augmentation + 5 Seeds

개선사항:
  1. Input noise augmentation: 학습 시 X에 σ=1mm Gaussian noise 추가 → 일반화 향상
  2. N_SEEDS 3→5: 모델 평균 효과 강화
  3. Hard-sample focused sampling: curvature 높은 샘플 오버샘플링

Pseudo-labeling 실패 이유: pseudo 잔차 std=0.17cm vs 실제 1.42cm → 모델 bias 발생
"""
...
# 곡률 기반 하드 샘플 가중치 (오버샘플링용)
kappa_train = build_scalar(X_train, noise_p, noise_s, noise_l)
# max_curvature는 build_scalar에서 log1p 적용된 값 → 그대로 사용
max_curv = np.exp(scal_tr[:, -5]) - 1  # log1p 역산 근사
hard_mask = max_curv > np.percentile(max_curv, 80)
print(f'Hard samples (top 20% curvature): {hard_mask.sum():,}')
...
N_FOLDS, N_SEEDS, FULL_EP, FULL_ESTOP = 5, 5, 400, 60  # seeds: 3→5, ep 350→400
NOISE_STD = 0.001  # 입력 노이즈 1mm
...
print(f'\n{"="*55}')
print(f'[v3_aug 결과 vs v3_D 기준]')
print(f'  OOF R-Hit:      {oof_rh:.4f}  (v3_D: 0.6659)')
print(f'  Cal OOF:        {best_cal:.4f}')
print(f'  alpha:          ({best_a[0]:.2f},{best_a[1]:.2f},{best_a[2]:.2f})')
...
fname = f'sub_v3_aug_5seeds_noise1mm_OOF{oof_rh:.4f}.csv'
