# -*- coding: utf-8 -*-
"""
train_v12_adaptive_kalman.py — End-to-End Adaptive Kalman Filter

Changes from v10:
  1. Entirely replaced static kalman_predict with a PyTorch Differentiable Kalman Filter.
  2. The process noise (Q) and observation noise (R) are dynamically predicted by a small GRU!
  3. The main model (GRUFlex) predicts body-frame residuals, which are rotated back to world frame INSIDE the PyTorch module.
  4. The model directly learns to minimize error against y_train (world frame).
"""
...
CKPT_DIR  = str(pathlib.Path(__file__).parent / 'cache' / 'v12_ckpt')
...
def kalman_predict_40ms(X, sigma_obs=0.3e-3, sigma_proc=1.0):
    """Kalman prediction for t+40ms (one DT step ahead of last observation)"""
    N, T, _ = X.shape
    F  = np.array([[1,DT],[0,1]])
    Q  = sigma_proc**2 * np.array([[DT**4/4, DT**3/2],[DT**3/2, DT**2]])
    R  = sigma_obs**2; pred = np.zeros((N,3))
    for j in range(3):
        ...
        for t in range(1,T):
            ...
            inn=z[:,t]-s[:,0]; S=P[0,0]+R; K=P[:,0]/S
            s=s+np.outer(inn,K); P=P-np.outer(K,P[0,:])
        pred[:,j]=(s@F.T)[:,0]   # 40ms = 1 DT ahead
    return pred
...
# Base static Kalman just for auxiliary targets & reference
...
print(f'Static Kalman R-Hit = {r_hit(kal_tr_best, y_train):.4f}')

# ─── Torsion Features ────────────────────────────────────────────
def build_torsion(X):
    """
    3D curve torsion τ: how much the trajectory twists out of its osculating
    plane. Curvature κ measures bending; torsion τ measures 3D spiral twist.
    Formula: τ = [(v × a) · j] / |v × a|²
    Returns: tau_seq (N,11,1), tau_scal (N,5)
    """
    N = X.shape[0]
    Xd = X.astype(np.float64)
    # Central-diff velocity at timesteps 1..9
    v = (Xd[:, 2:, :] - Xd[:, :-2, :]) / (2*DT)                    # (N,9,3)
    # Central-diff acceleration at timesteps 1..9
    a = (Xd[:,2:,:] - 2*Xd[:,1:-1,:] + Xd[:,:-2,:]) / DT**2        # (N,9,3)
    # Central-diff jerk at timesteps 2..8
    j = (a[:, 2:, :] - a[:, :-2, :]) / (2*DT)                      # (N,7,3)

    # Align v,a to timesteps 2..8
    cross    = np.cross(v[:, 1:-1, :], a[:, 1:-1, :])               # (N,7,3)
    cross_sq = (cross**2).sum(-1)                                    # (N,7)
    dot      = (cross * j).sum(-1)                                   # (N,7)

    # τ = 0 when motion is (nearly) straight (|v×a|≈0)
    tau = np.where(cross_sq > 1e-16, dot / (cross_sq + 1e-20), 0.0)
    tau = np.clip(tau, -1000., 1000.)   # (N,7) at timesteps 2..8

    # Symmetric log: preserves sign, compresses magnitude
    tau_log = np.sign(tau) * np.log1p(np.abs(tau))

    # Per-timestep seq channel (N,11): values at 2..8, edge-hold padded
    tau_seq = np.zeros((N, 11), dtype=np.float32)
    tau_seq[:, 2:9] = tau_log.astype(np.float32)
    tau_seq[:, 0] = tau_seq[:, 2]; tau_seq[:, 1] = tau_seq[:, 2]
    tau_seq[:, 9] = tau_seq[:, 8]; tau_seq[:, 10] = tau_seq[:, 8]

    # Scalar summaries (N,5)
    signs   = np.sign(tau)
    sign_ch = (signs[:, 1:] != signs[:, :-1]).astype(np.float32).sum(1)
    tau_scal = np.column_stack([
        tau_log.mean(1),
        tau_log.std(1),
        tau_log[:, -1],                # most recent torsion (ts=8)
        np.log1p(np.abs(tau).max(1)),  # log max absolute torsion
        sign_ch,                       # # of twist direction reversals
    ]).astype(np.float32)

    return tau_seq[:, :, None], tau_scal   # (N,11,1), (N,5)
...
def build_seq_curv(X, tau_seq=None):
    N = X.shape[0]
    rel = X - X[:,-1:]
    ...
    parts = [rel, vp, ap, speed, curvature, angular_vel, turn_r]
    if tau_seq is not None:
        parts.append(tau_seq)
...
print('피처 빌딩...')
tau_seq_tr, tau_scal_tr = build_torsion(X_train)
tau_seq_te, tau_scal_te = build_torsion(X_test)
...
# scal: 27 + 19 + 27 + 5(torsion) = 78
scal_full_tr = np.concatenate([scal_tr, t3_tr, wav_tr, tau_scal_tr], -1)
scal_full_te = np.concatenate([scal_te, t3_te, wav_te, tau_scal_te], -1)
# seq: 13 + 1(torsion) = 14
seq_curv_tr  = build_seq_curv(X_train, tau_seq_tr)
seq_curv_te  = build_seq_curv(X_test,  tau_seq_te)
...
# Target for End-to-End is just y_train (World Frame)
target_main = y_train.astype(np.float32)
...
kal_40ms_tr  = kalman_predict_40ms(X_train, so_best, sp_best)
y_40ms_proxy = X_train[:,-1] + (y_train - X_train[:,-1]) * 0.5
target_mid   = rotate_xy(y_40ms_proxy - kal_40ms_tr, theta_tr).astype(np.float32)
...
class AdaptiveKalman(nn.Module):
    def __init__(self):
        ...
        self.fc_q = nn.Linear(16, 1) # log sigma_proc
        self.fc_r = nn.Linear(16, 1) # log sigma_obs
        
        self.register_buffer('F_mat', torch.tensor([[1., DT],[0., 1.]], dtype=torch.float32))
        self.register_buffer('Fp_mat', torch.tensor([[1., T_PRED],[0., 1.]], dtype=torch.float32))
        self.register_buffer('Q_base', torch.tensor([[DT**4/4, DT**3/2],[DT**3/2, DT**2]], dtype=torch.float32))

    def forward(self, X):
        B, T, _ = X.shape
        out, _ = self.gru(X - X[:, 0:1, :]) 
        
        log_sp = self.fc_q(out).squeeze(-1).clamp(-5, 5) # Process noise variance limits
        log_so = self.fc_r(out).squeeze(-1).clamp(-12, -2) # Obs noise variance limits
        sp = torch.exp(log_sp)
        so = torch.exp(log_so)
        ...
        for j in range(3):
            z = X[:, :, j] 
            s = torch.zeros(B, 2, device=X.device)
            s[:, 0] = z[:, 0]
            P = torch.eye(2, device=X.device).unsqueeze(0).repeat(B, 1, 1) 
            
            for t in range(1, T):
                s = torch.matmul(s, self.F_mat.T)
                Q = self.Q_base.unsqueeze(0) * (sp[:, t]**2).view(B, 1, 1)
                P = torch.matmul(self.F_mat, torch.matmul(P, self.F_mat.T)) + Q
                
                inn = z[:, t] - s[:, 0]
                S_k = P[:, 0, 0] + so[:, t]**2
                K = P[:, :, 0] / (S_k.unsqueeze(1) + 1e-8)
                
                s = s + K * inn.unsqueeze(1)
                P = P - torch.matmul(K.unsqueeze(2), P[:, 0, :].unsqueeze(1))
                
            pred = torch.matmul(s, self.Fp_mat.T)[:, 0]
            preds.append(pred)
...
class GRUFlex(nn.Module):
    def __init__(self, seq_dim=SEQ_DIM, scal_dim=SCAL_DIM,
        ...
        self.head_M    = nn.Linear(fc_hidden//2, 3)
        self.clip = main_clip
...
class EndToEndModel(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.gruflex = GRUFlex(**kwargs)
        
    def forward(self, seq, scal, X_raw, theta_yaw):
        kal_pred = self.kalman(X_raw) 
        rot_res, oF, oW, oMid = self.gruflex(seq, scal)
        
        c = torch.cos(theta_yaw); s = torch.sin(theta_yaw)
        res_x = rot_res[:, 0]*c - rot_res[:, 1]*s
        res_y = rot_res[:, 0]*s + rot_res[:, 1]*c
        res_z = rot_res[:, 2]
        world_res = torch.stack([res_x, res_y, res_z], dim=-1)
        
        final_pred = kal_pred + world_res
        return final_pred, oF, oW, oMid
...
# ─── Training Config ─────────────────────────────────────────────
CFG = dict(lr=0.000989, wd=0.000886, batch=256, lF=0.30, lW=0.30, lM=0.10)
...
for fi, (tri, vai) in enumerate(kf.split(np.arange(len(X_train)))):
    ...
    if os.path.exists(ckpt_oof) and os.path.exists(ckpt_te) and os.path.exists(ckpt_rh):
        ...
        rh = float(open(ckpt_rh).read())
        fold_rh.append(rh)
        print(f'[Fold {fi+1}/{N_FOLDS}] loaded from checkpoint  R-Hit={rh:.4f}')
        continue
    ...
    # Needs raw X and theta for End-to-End
    X_raw_tr = tt(X_train[tri]); th_tr = tt(theta_tr[tri])
    X_raw_va = tt(X_train[vai]); th_va = tt(theta_tr[vai])
    X_raw_te = tt(X_test);       th_te = tt(theta_te)

    tm   = tt(target_main[tri])
    tf   = tt(aux_F[tri])
    tw   = tt(aux_W[tri])
    tmid = tt(target_mid[tri])

    seed_oof, seed_te = [], []
    for s in range(N_SEEDS):
        torch.manual_seed(s); np.random.seed(s)
        m = EndToEndModel().to(device)
        ...
        lF, lW, lM, bs = CFG['lF'], CFG['lW'], CFG['lM'], CFG['batch']
        ...
        for ep in range(FULL_EP):
            ...
            for i in range(0, n, bs):
                ...
                # Forward requires raw X and theta now
                om, oF, oW, oMid = m(st_base[idx] + noise, sc_[idx], X_raw_tr[idx], th_tr[idx])
                loss = (loss_combo_focal(om, tm[idx])
                        + lF * loss_euclid(oF, tf[idx])
                        + lW * loss_euclid(oW, tw[idx])
                        + lM * loss_euclid(oMid, tmid[idx]))
                loss.backward()
            ...
            m.eval()
            with torch.no_grad(): 
                # om is already in world frame! No rotation needed.
                final_pred, _, _, _ = m(sv, scv, X_raw_va, th_va)
            
            rh = r_hit(final_pred.cpu().numpy(), y_train[vai])
            if rh > best_rh:
                best_rh = rh
        ...
        m.eval()
        with torch.no_grad():
            ov2, _, _, _ = m(sv,  scv, X_raw_va, th_va)
            te_, _, _, _ = m(ste, sce, X_raw_te, th_te)
        # Directly store final world frame prediction
    ...
    rh  = r_hit(vr, y_train[vai])
    fold_rh.append(rh)
    print(f'  Fold {fi+1} OOF R-Hit: {rh:.4f}')

    np.save(ckpt_oof, vr)
    np.save(ckpt_te,  tr_)
    with open(ckpt_rh, 'w') as f: f.write(str(rh))

# ─── Final Evaluation ────────────────────────────────────────────
# oof_rot already stores final prediction!
oof_pred = oof_rot
...
for ax in np.arange(0.9, 1.1, 0.05):
    for ay in np.arange(0.9, 1.1, 0.05):
        for az in np.arange(0.9, 1.1, 0.05):
            a = np.array([ax, ay, az])
            rc = r_hit(oof_pred * a, y_train)
            if rc > best_cal: best_cal, best_a = rc, a.copy()

print(f'\n{"="*55}')
print(f'[v12 Adaptive Kalman 결과]')
...
test_pred = test_rot
fname = f'sub_v12_adaptive_kalman_OOF{oof_rh:.4f}_Cal{best_cal:.4f}.csv'
