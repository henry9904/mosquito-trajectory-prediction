# -*- coding: utf-8 -*-
"""
train_v10.py — v3_aug + Torsion Features + Multi-step Auxiliary

Changes from v3_aug (current best OOF=0.6669):
  1. Torsion τ: 3D curve torsion channel in seq (seq_dim: 13→14)
     + 5 scalar summary features (scal_dim: 73→78)
     Curvature κ measures bending; torsion τ measures 3D twist (spiral vs flat)
  2. Multi-step aux head_M: predicts t+40ms proxy residual (lM=0.10)
...
CKPT_DIR  = str(pathlib.Path(__file__).parent / 'cache' / 'v10_ckpt')
...
fname = f'sub_v10_torsion_multistep_OOF{oof_rh:.4f}_Cal{best_cal:.4f}.csv'
