# -*- coding: utf-8 -*-
"""
train_v11_multi_kalman.py — v10 + Multi-Sigma Kalman Features

Changes from v10:
  1. Generate Kalman predictions using 4 different sigma_obs values [1e-4, 2.67e-4, 1e-3, 3e-3].
  2. Rotate these relative predictions to body frame and append to scalar features.
  3. This gives the GRU access to how different physics assumptions extrapolate the trajectory.
     Proxy: linear interpolation (X[-1] + y_train) / 2 → Kalman_40ms residual
  3. Fold checkpointing: saves per-fold results for crash recovery
"""
...
CKPT_DIR  = str(pathlib.Path(__file__).parent / 'cache' / 'v11_ckpt')
...
# ─── Multi-Kalman Features ───────────────────────────────────────
SIGMAS = [1e-4, 2.67e-4, 1e-3, 3e-3]
def build_multi_kalman(X, theta):
    feats = []
    for sig in SIGMAS:
        kal_p = kalman_predict(X, sig, 1.0)
        rel_p = kal_p - X[:,-1]
        rot_p = rotate_xy(rel_p, theta)
        feats.append(rot_p)
    return np.concatenate(feats, axis=-1).astype(np.float32)

print('멀티 칼만 피처 빌딩...')
mkal_tr = build_multi_kalman(X_train, theta_tr)
mkal_te = build_multi_kalman(X_test, theta_te)

scal_full_tr = np.concatenate([scal_full_tr, mkal_tr], -1)
scal_full_te = np.concatenate([scal_full_te, mkal_te], -1)
...
# Multi-step t+40ms auxiliary target (linear interp proxy)
...
print(f'target_mid stats: mean={target_mid.mean():.5f} std={target_mid.std():.5f}')
...
class GRUFlex(nn.Module):
    def __init__(self, seq_dim=SEQ_DIM, scal_dim=SCAL_DIM,
        ...
        self.head_M    = nn.Linear(fc_hidden//2, 3)   # t+40ms auxiliary
        self.clip = main_clip
    ...
    for s in range(N_SEEDS):
        ...
        for ep in range(FULL_EP):
            ...
            for i in range(0, n, bs):
                ...
                om, oF, oW, oMid = m(st_base[idx] + noise, sc_[idx])
            ...
            m.eval()
            with torch.no_grad(): ov,_,_,_ = m(sv, scv)
...
fname = f'sub_v11_multi_kalman_OOF{oof_rh:.4f}_Cal{best_cal:.4f}.csv'
